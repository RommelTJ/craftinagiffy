...
if "x%GRAILS_AGENT_CACHE_DIR%" == "x" set GRAILS_AGENT_CACHE_DIR=%SHORTHOME%/.grails/2.4.3/
...
if "%GRAILS_OPTS%" == "" (
	...
	if not "%GRAILS_NO_PERMGEN%" == "1" (
		set GRAILS_OPTS=-server -Xms96m -Xmx128m  -XX:MaxMetaspaceSize=256m -XX:+CMSClassUnloadingEnabled -XX:+UseConcMarkSweepGC -Dfile.encoding=UTF-8
	)
)
...
set STARTER_CLASSPATH=wrapper/grails-wrapper-runtime-2.4.3.jar;wrapper;.
...
set JAVA_OPTS=%JAVA_OPTS% -Dgrails.version=2.4.3
